...
function formatClaudeModelName(value) {
  ...
  const title = (part) => part.charAt(0).toUpperCase() + part.slice(1).toLowerCase()
...
function claudeModelInfo(modelId) {
  ...
  const stripped = raw.replace(/\[1m\]$/i, '').replace(/-20\d{6}(?:-v\d+)?$/i, '')
  const known = CLAUDE_MODELS.find((model) =>
    model.id === raw
    || model.id === alias
    || model.id === stripped
    || raw === model.id
  ...
  return { provider: 'anthropic', id: alias || raw, name: formatClaudeModelName(alias || stripped || raw) }
}
...
function parseClaudeUsageText(stdout) {
  ...
  return [...resultText.matchAll(/^(Current session|Current week[^:]*):\s*(\d+(?:\.\d+)?)% used(?:\s*·\s*resets\s+(.+))?$/gim)]
    .map((match) => ({
      label: /^Current session$/i.test(match[1]) ? 'Current session' : 'Current week',
...
function loadClaudeUsage() {
  return new Promise((resolve) => {
    const child = spawn(resolveClaudeExecutable(), [
      '-p',
      '/usage',
      '--output-format', 'json',
    ...
    const timeout = setTimeout(() => finish({ ok: false, error: 'Claude usage check timed out.' }), 25_000)
    ...
    child.once('exit', (code) => {
      ...
      if (windows.length > 0) {
        finish({ ok: true, usage: { available: true, provider: 'Claude', windows, updatedAt: new Date().toISOString() } })
        return
      }
      finish({
        ok: code === 0,
        usage: { available: false, provider: 'Claude', windows: [] },
        ...(code === 0 ? {} : { error: stderr.trim() || `Claude usage check exited with code ${code}` }),
      })
    })
  })
}
...
function normalizeHistoryEntry(entry) {
  ...
  if (entry.type === 'assistant') {
    const content = Array.isArray(message.content) ? message.content.map((part) => {
      if (part?.type === 'tool_use') {
        return { type: 'toolCall', id: part.id, name: part.name, arguments: part.input ?? {} }
      }
      return part
    }) : []
  ...
  if (/<local-command-caveat>|<command-name>|<command-message>/.test(text)) return []
  if (Array.isArray(content) && content.some((part) => part?.type === 'tool_result')) {
    return content.filter((part) => part?.type === 'tool_result').map((part) => ({
      role: 'toolResult',
  ...
  return [{ role: 'user', content: typeof content === 'string' ? [{ type: 'text', text: content }] : content, timestamp }]
}

export function messagesFromClaudeLog(contents) {
  return String(contents || '').split('\n').filter(Boolean).flatMap((line) => {
    try { return normalizeHistoryEntry(JSON.parse(line)) } catch { return [] }
  })
}
...
export class ClaudeAgentProcess {
  ...
  async start(cwd, options = {}) {
    ...
    this.thinkingLevel = options.thinkingLevel || this.thinkingLevel || DEFAULT_CLAUDE_EFFORT
    if (options.sessionPath) {
      ...
      this.model = options.model?.id
        ? claudeModelInfo(options.model.id) ?? { provider: 'anthropic', id: options.model.id, name: options.model.name || options.model.id }
        : runtime.model
          ? claudeModelInfo(runtime.model) ?? { provider: 'anthropic', id: runtime.model, name: runtime.model }
          : null
      if (!options.thinkingLevel && runtime.effort) this.thinkingLevel = runtime.effort
    } else {
      this.model = options.model?.id
        ? claudeModelInfo(options.model.id) ?? { provider: 'anthropic', id: options.model.id, name: options.model.name || options.model.id }
  ...
  async spawnProcess(extraArgs = []) {
    ...
    const args = [
      ...
      '--include-hook-events',
      '--verbose',
      '--append-system-prompt', CO_PARTNER_PROMPT,
    ]
    ...
    child.once('exit', (code, signal) => {
      ...
      if (this.status !== 'stopped') {
        const message = code && code !== 0 ? `Claude exited with code ${code}` : undefined
  ...
  prompt(message, images) { return this.sendTurn(message, images, 'prompt') }
  steer(message, images) { return this.sendTurn(message, images, 'steer') }
  followUp(message, images) { return this.sendTurn(message, images, 'follow_up') }

  sendTurn(message, images, kind = 'prompt') {
    if (!this.process) return Promise.resolve({ ok: false, error: 'Claude process is not running' })
  ...
  async setModel(_provider, modelId) {
    this.model = claudeModelInfo(modelId) ?? { provider: 'anthropic', id: modelId, name: modelId }
    const result = await this.restart()
    return result.ok ? { ok: true, data: this.model, state: await this.getState() } : result
  }
  ...
  async forkAt() {
    if (!this.sessionId) return { ok: false, error: 'No Claude session is available to fork.' }
    const messages = await this.getMessages()
  ...
  getThinkingLevels() { return Promise.resolve({ ok: true, levels: CLAUDE_EFFORT_LEVELS }) }

  getUsage(force = false) {
    const now = Date.now()
    if (!force && usageCache.result && now - usageCache.at < USAGE_TTL_MS) return Promise.resolve(usageCache.result)
    if (usageCache.promise) return usageCache.promise
    usageCache.promise = loadClaudeUsage().then((result) => {
      if (result?.ok) {
      ...
      usageCache.promise = undefined
      return { ok: false, error: String(error?.message ?? error), usage: { available: false, provider: 'Claude', windows: [] } }
    })
  ...
  handleLine(line) {
    ...
    try { event = JSON.parse(line) } catch {
      this.emit({ type: 'claude_raw_line', sessionKey: this.sessionKey, raw: line })
      return
    }
    ...
    if (event.type === 'system' && event.subtype === 'init') {
      ...
      if (authSource && authSource !== 'none') {
        const error = new Error(`Claude subscription auth required; received ${authSource}.`)
        this.emit({ type: 'stderr', sessionKey: this.sessionKey, message: error.message })
      ...
      if (typeof event.model === 'string') this.model = claudeModelInfo(event.model) ?? { provider: 'anthropic', id: event.model, name: event.model }
      if (Array.isArray(event.tools)) this.availableTools = event.tools.filter((tool) => typeof tool === 'string')
      this.skills = new Set(Array.isArray(event.skills) ? event.skills.filter((skill) => typeof skill === 'string') : [])
      ...
      ].filter((command) => typeof command === 'string'))].sort((left, right) => left.localeCompare(right))
      this.sessionFile = this.sessionFile ?? expectedSessionPath(this.cwd, this.sessionId)
      this.emit({
        type: 'claude_init',
        sessionKey: this.sessionKey,
    ...
    if (event.type === 'assistant' && event.message) {
      ...
      if (event.parent_tool_use_id && !this.seenSubagents.has(event.parent_tool_use_id)) {
        this.seenSubagents.add(event.parent_tool_use_id)
        this.emit({ type: 'subagent_start', sessionKey: this.sessionKey, parentToolUseId: event.parent_tool_use_id })
      }
      const activeStream = this.activeStreams.get(source) ?? this.beginMessageStream(source)
      ...
      const message = {
        ...event.message,
        content: content.map((part) => part?.type === 'tool_use'
          ? { type: 'toolCall', id: part.id, name: part.name, arguments: part.input ?? {} }
          : part),
        timestamp: Date.now(),
      }
      this.emit({ type: 'message_end', sessionKey: this.sessionKey, streamKey: activeStream.key, message })
      this.messageCount += 1
    ...
    if (event.type === 'user' && event.message) {
      ...
      for (const part of content) {
        ...
        this.emit({
          ...
          result: { content: [{ type: 'text', text: contentText(part.content) }], details: event.toolUseResult ?? {} },
          isError: Boolean(part.is_error),
        })
      }
      return
    ...
    if (event.type === 'result') {
      ...
      pending?.resolve(ok ? { ok: true, data: event } : { ok: false, error: String(event.result ?? 'Claude request failed') })
      this.emit({
        type: 'turn_result',
        sessionKey: this.sessionKey,
    ...
    if (event.type === 'rate_limit_event') {
      this.emit({ type: 'rate_limit_event', sessionKey: this.sessionKey, rate_limit_info: event.rate_limit_info })
      return
    }
  ...
  handleStreamEvent(streamEvent, parentToolUseId) {
    ...
    if (streamEvent.type === 'message_start') {
      if (parentToolUseId && !this.seenSubagents.has(parentToolUseId)) {
        ...
        this.emit({ type: 'subagent_start', sessionKey: this.sessionKey, parentToolUseId })
      }
    ...
    if (streamEvent.type === 'content_block_start') {
      ...
      activeStream.blocks.set(index, { type: block.type, text: block.text ?? block.thinking ?? '' })
      return
    }
    if (streamEvent.type === 'content_block_delta') {
      const activeStream = this.activeStreams.get(source) ?? this.beginMessageStream(source)
      const delta = streamEvent.delta ?? {}
      const block = activeStream.blocks.get(index) ?? { type: delta.type === 'thinking_delta' ? 'thinking' : 'text', text: '' }
      ...
      this.emit({
        ...
        assistantMessageEvent: {
          type: delta.type === 'thinking_delta' ? 'thinking_delta' : 'text_delta',
          contentIndex: index,
          delta: text,
        },
      })
  ...
  beginMessageStream(source) {
    ...
    this.emit({ type: 'turn_start', sessionKey: this.sessionKey, streamKey: stream.key })
    return stream
  }
  ...
  async terminateProcess(error) {
    ...
    await new Promise((resolve) => {
      ...
      const timeout = setTimeout(() => { this.signalProcess(child, 'SIGKILL'); resolve() }, 1200)
...
export class ClaudeAgentPool {
  ...
  get(sessionKey) {
    ...
    if (!agent) { agent = new ClaudeAgentProcess(sessionKey); this.agents.set(sessionKey, agent) }
    return agent
  }
