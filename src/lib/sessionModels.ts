...
export function sessionModelIds(session: Pick<ResumeSession, 'models' | 'lastModel'>): string[] {
...
export function sessionUsesModel(session: Pick<ResumeSession, 'models' | 'lastModel'>, modelId: string): boolean {
  if (!modelId) return true
  return sessionModelIds(session).includes(modelId)
}

export function uniqueSessionModels(sessions: Array<Pick<ResumeSession, 'models' | 'lastModel'>>): { id: string; label: string }[] {
  const ids = new Set<string>()
  for (const session of sessions) {
    for (const id of sessionModelIds(session)) ids.add(id)
  }
  return [...ids]
    .sort((a, b) => formatSessionModelName(a).localeCompare(formatSessionModelName(b)))
